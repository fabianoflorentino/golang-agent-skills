...
Map database rows to Go structs with `db` tags for sqlx or `pgx.CollectRows` for pgx. Handle NULLable columns with pointer fields, `sql.NullXxx` types, or `COALESCE` in SQL.
