...
Unit tests verify business logic with mocks. Integration tests verify SQL correctness against a real database. Gate integration tests with build tags so `go test ./...` skips them by default.
...
→ See `fabianoflorentino/golang-agent-skills@golang-testing` skill for general test patterns and CI configuration.
