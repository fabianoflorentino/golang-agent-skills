...
Go treats errors as ordinary values implementing the `error` interface. Errors are returned, not thrown. Every function that can fail returns an `error` as its last return value, and every caller must check it.
