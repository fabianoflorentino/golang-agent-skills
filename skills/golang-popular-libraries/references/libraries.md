...
Curated list of widely-used Go libraries by domain. Prefer the standard library when it covers the need — external dependencies add maintenance burden and security surface.

## Web frameworks

- **Gin** (<https://github.com/gin-gonic/gin>) — High-performance HTTP web framework with minimalist API. Up to 40x faster than some alternatives. Great for REST APIs and microservices.
- **Echo** (<https://github.com/labstack/echo>) — Minimalist, extensible web framework. Clean middleware system, excellent performance. Good for REST APIs and traditional web apps.
...
- **env** (<https://github.com/caarlos0/env>) — Parse environment variables into Go structs with defaults. Simple, type-safe, no struct tags.
