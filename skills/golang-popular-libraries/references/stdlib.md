...
- **encoding/json/v2** (default JSON implementation as of Go 1.27; introduced experimental behind `GOEXPERIMENT=jsonv2` in Go 1.25) — Next-generation JSON encoding/decoding, now the default underneath `encoding/json`. Stricter than v1: rejects duplicate object keys and invalid UTF-8. → See `fabianoflorentino/golang-agent-skills@golang-modernize` skill for migration guidance and the `GOEXPERIMENT=nojsonv2` rollback.
