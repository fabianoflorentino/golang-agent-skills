...
- [Testing error codes](#testing-error-codes)
  - [Table-driven code checks](#table-driven-code-checks)
...
`bufconn` supplies in-memory listeners that run the real gRPC stack — serialization, interceptors, metadata — with no sockets and no leaked ports.
...
Register the real interceptors on the test server so the test covers them:
...
Clients branch on status codes, so assert the code rather than a string message. Decode with `status.FromError`:
...
func TestGetUser_Errors(t *testing.T) {
    ...
    tests := []struct {
        ...
        {name: "empty user ID", req: &pb.GetUserRequest{UserId: ""}, wantCode: codes.InvalidArgument},
        {name: "user not found", req: &pb.GetUserRequest{UserId: "nonexistent"}, wantCode: codes.NotFound},
    }
...
Send the batch, then read the final response with `CloseAndRecv`:
...
Prove the auth interceptor reads the outgoing metadata:
...
Pin a short timeout against a handler that sleeps, and expect `DeadlineExceeded`:
...
When the test must hit real dependencies, isolate it behind a build tag so `go test ./...` stays hermetic:
...
func TestUserService_Integration(t *testing.T) {
    ...
    // Full roundtrip against a live server
    ...
    got, err := client.GetUser(context.Background(), &pb.GetUserRequest{UserId: created.User.Id})
    if err != nil {
