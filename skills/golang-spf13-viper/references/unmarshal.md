...
Always use `mapstructure` tags. Without them, mapstructure falls back to case-insensitive field name matching, which works for simple cases but silently fails for nested structs where the outer key uses an underscore, unexported fields, or fields where the Go name does not match the config key.
