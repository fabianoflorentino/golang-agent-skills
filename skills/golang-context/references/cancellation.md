...
`context.Context` propagates cancellation signals through a call tree. When a request is cancelled or times out, every downstream operation that checks the context stops promptly instead of running to completion.

## Cancellation

`context.WithCancel` returns a derived context and a `cancel` function. When `cancel()` is called, the context's `Done()` channel closes, signaling all listeners to stop.
...
Every `WithCancel`, `WithTimeout`, and `WithDeadline` creates cancellation state; timeout and deadline contexts also use timer resources. `cancel()` must be called on all control-flow paths unless the function explicitly returns or transfers ownership of both the context and cancel function. In ordinary scoped work, defer cancel immediately.
...
If a parent context has a 2s timeout and you create a child with 10s, the child still expires at 2s. The shorter deadline always wins.
...
Creates a child context that is not cancelled when the parent is. Use for background work that must continue after the request completes — async logging, audit trails, or enqueuing follow-up tasks.
...
Without `WithoutCancel`, you would choose between `ctx` (cancelled when the handler returns, killing background work) and `context.Background()` (loses trace_id and other values). `WithoutCancel` preserves values but detaches cancellation.
