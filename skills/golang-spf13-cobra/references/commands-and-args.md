...
Cobra commands have five run hooks executed in fixed order: `PersistentPreRunE` → `PreRunE` → `RunE` → `PostRunE` → `PersistentPostRunE`. Each `*E` hook returns `error`. The non-`*E` variants have signature `func(cmd *cobra.Command, args []string)` — they cannot signal failure without `os.Exit` or panic. Always use the `*E` variants.
