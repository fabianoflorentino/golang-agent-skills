...
Cobra delegates all flag parsing to `github.com/spf13/pflag`. `cobra.Command` exposes two `*pflag.FlagSet`s: `cmd.Flags()` (local flags, only available on this command) and `cmd.PersistentFlags()` (inherited by all subcommands).
...
cmd.Flags().Duration / DurationVar
cmd.Flags().StringSlice / StringSliceVar
cmd.Flags().StringArray / StringArrayVar
